...
* Have removed the dependency on Hoe[https://github.com/seattlerb/hoe]. The
  build is now vanilla gemspec based.
...
* Updated the 'add' method to allow the optional specification of an insertion
  position in the child array.

* Added a new method 'detached_subtree_copy' to allow cloning the entire tree
...
* Converted all CamelCase method names to the canonical ruby_method_names
  ...
  throw a Deprecated warning. The support for old CamelCase methods will go away
...
* Added new methods for converting from/to JSON formats. Thanks to Dirk
...
* Added fixes for root related edge conditions to 'isOnlyChild?',
  'nextSibling', 'previousSibling' and 'remove' methods.
...
* Fixed the
  ...
  where the depth method was actually returning height+1 (not the depth).
